...
# Download data for user-specified ticker and SPY from start date to present
...
class MeanReversionRSIStrategy(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if current_price < self.sma[-1] and current_rsi < self.rsi_oversold:
                self.buy()
                self.entry_price = current_price
                trades.append({
                    'date_entered': self.data.index[-1],
                    'price_entered': current_price,
                    'type': 'buy',
                    'amount_paid': self.position.size * current_price,
                    'rsi_entry': current_rsi
                })
            elif current_price > self.sma[-1] and current_rsi > self.rsi_overbought:
                self.sell()
                self.entry_price = current_price
                trades.append({
                    'date_entered': self.data.index[-1],
            ...
            if self.position.is_long:
                if current_price > self.sma[-1] or current_rsi > self.rsi_overbought:
                    self.position.close()
                    self.update_trade_exit(current_price, current_rsi)
                elif current_price <= self.entry_price * 0.5:  # 50% stop loss
                    self.position.close()
                    self.update_trade_exit(current_price, current_rsi)
            elif self.position.is_short:
                if current_price < self.sma[-1] or current_rsi < self.rsi_oversold:
                    self.position.close()
                    self.update_trade_exit(current_price, current_rsi)
    ...
    def update_trade_exit(self, exit_price, exit_rsi):
        trades[-1]['date_exited'] = self.data.index[-1]
        trades[-1]['price_exited'] = exit_price
        trades[-1]['rsi_exit'] = exit_rsi
        if trades[-1]['type'] == 'buy':
            pl_pct = (exit_price - trades[-1]['price_entered']) / trades[-1]['price_entered'] * 100
        else:  # short
            pl_pct = (trades[-1]['price_entered'] - exit_price) / trades[-1]['price_entered'] * 100
        trades[-1]['pl_pct'] = pl_pct
