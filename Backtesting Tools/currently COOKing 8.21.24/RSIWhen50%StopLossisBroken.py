...
variable_data = pdr.get_data_fred(user_variable, start="2024-01-01", end=end_date)
...
class CombinedStrategy(Strategy):
    ...
    def init(self):
        ...
        self.use_rsi_strategy = False
        self.days_outside_range = 0
        self.entry_price = 0
    ...
    def next(self):
        ...
        if not self.use_rsi_strategy:
            # Check if price is outside the range
            if current_price < lower_price or current_price > upper_price:
                self.days_outside_range += 1
            else:
                self.days_outside_range = 0
            
            # Switch to RSI strategy if outside range for more than 15 days
            if self.days_outside_range > self.breakout_days:
                self.use_rsi_strategy = True
                if self.position:
                    self.position.close()
                return
            
            # Dollar Range Strategy
...
# Run backtest
bt = Backtest(ticker_data, CombinedStrategy, cash=10000, commission=.002)
results = bt.run()
...
if results['Return [%]'] > buy_and_hold_returns:
    print("The Combined Strategy outperformed Buy-and-Hold.")
else:
    print("Buy-and-Hold outperformed the Combined Strategy.")
