...
def binomial_pricing(S, K, T, r, sigma, n=100):
    if T <= 0:
    ...
    for i in range(n + 1):
        call_prices[i, n] = max(0, S * (u ** (n - i)) * (d ** i) - K)
        put_prices[i, n] = max(0, K - S * (u ** (n - i)) * (d ** i))

    for j in range(n - 1, -1, -1):
        for i in range(j + 1):
            call_prices[i, j] = np.exp(-r * dt) * (p * call_prices[i, j + 1] + (1 - p) * call_prices[i + 1, j + 1])
            put_prices[i, j] = np.exp(-r * dt) * (p * put_prices[i, j + 1] + (1 - p) * put_prices[i + 1, j + 1])
...
def analyze_price_range(S, K, T, r, sigma):
    ...
    for price in price_range:
        call_price, put_price = binomial_pricing(price, K, T, r, sigma)
...
def main():
    while True:
        try:
            ...
            call_price, put_price = binomial_pricing(S, K, T, r, sigma)
